...
<body class="d-flex flex-column h-100">
	...
	<div class="board_section">
			...
			<table class="board_table" border="1">
				<thead class="board_thead">
					<tr class="board_tr">
						...
						<th class="board_th"><a href="mentor?&sort=view">조회수↑</a></th>
						<th class="board_th"><a href="mentor?sort=recommend">추천수↑</a></th>
					</tr>
				</thead>
				<tbody class="board_tbody">
					<c:forEach items="${boardMentorList}" var="list" varStatus="status">
						<tr class="board_tr">
							<td class="board_td">${list.bdIdx}</td>
							<td class="board_td"><a
								href="mentor/mentor-board-detail?bdIdx=${list.bdIdx}&userId=${list.userId}"><c:out
		...
		<div id="boardFooter">
				...
				<div id="pagingBox">
					...
					<c:if test="${startNum>1}">
						<i class="fas fa-chevron-left" id="leftArrow"
							onclick="location.href='mentor?p=${startNum-1}&searchCondition=${param.searchCondition}&searchKeyWord=${param.searchKeyWord}'"></i>
					</c:if>
					<c:if test="${startNum<=1}">
						<i class="fas fa-chevron-left" id="leftArrow"
						...
						<c:forEach var="i" begin="0" end="2">
							<c:if test="${(startNum+i) <= lastNum}">
								<button id="firstButton"
									onclick="location.href='mentor?p=${startNum+i}&searchCondition=${param.searchCondition}&searchKeyWord=${param.searchKeyWord}'">${startNum+i}</button>
							</c:if>
						</c:forEach>
					</div>
					<c:if test="${startNum+2<lastNum}">
						<i class="fas fa-chevron-right" id="rightArrow"
							onclick="location.href='mentor?p=${startNum+3}&searchCondition=${param.searchCondition}&searchKeyWord=${param.searchKeyWord}'"></i>
					</c:if>
					<c:if test="${startNum+2>=lastNum}">
						<i class="fas fa-chevron-right" id="rightArrow"
			...
			<div id="writerButton">
				<button type="button"
					onclick="location.href='mentor/mentor-board-create-form'">글작성</button>
			</div>
		</div>
	</div>
